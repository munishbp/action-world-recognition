"""
Training script for PredRNN on Something-Something V2.

Usage:
    python -m models.predrnn.train
    python -m models.predrnn.train --epochs 50 --batch-size 16 --num-frames 8
"""
...
from shared import get_dataloader, evaluate_model, save_results
...
def main():
    parser = argparse.ArgumentParser(description="Train PredRNN on SSv2")
    ...
    # Data (decode .webm directly via shared pipeline -- works on any platform)
    print("Loading data from video files (decord)...")
    train_loader = get_dataloader(
        split="train", batch_size=args.batch_size, num_frames=args.num_frames, num_workers=8
    )
    val_loader = get_dataloader(
        split="val", batch_size=args.batch_size * 2, num_frames=args.num_frames, num_workers=8
    )
    print(f"Train: {len(train_loader.dataset)} samples, Val: {len(val_loader.dataset)} samples")

    # Model
    model = PredRNNClassifier().to(device)
    ...
    print("Results saved to results/PredRNN_results.json")
