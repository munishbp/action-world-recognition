"""
Train TimeSformer-base on Something-Something V2.

Mirrors models/videomae/train.py: same checkpoint layout, metrics CSV,
final-eval-with-best.pt flow, and shared.evaluate_model output.

Starts from facebook/timesformer-base-finetuned-k400 (Kinetics-400 fine-tune,
400-class head) and replaces the head with a fresh 174-class linear layer.

Usage (from repo root):
    python models/timesformer/train.py
    python models/timesformer/train.py --batch-size 8 --epochs 15
    python models/timesformer/train.py --device cpu --batch-size 2 --num-workers 0 --smoke-test

Checkpoints: models/timesformer/checkpoints/
Results:     results/TimeSformer_results.json
"""
...
NUM_CLASSES = 174
PRETRAINED_ID = "facebook/timesformer-base-finetuned-k400"
...
def train_one_epoch(
    ...
    for i, batch in enumerate(tqdm(loader, desc="  Train", leave=False)):
        ...
        if batch is None:
            continue
        frames, labels = batch
        # shared loader returns (B, T, C, H, W) — TimeSformer wants exactly this. No permute.
...
def main():
    parser = argparse.ArgumentParser(description="Train TimeSformer-base on SSv2")
    ...
    # Fail fast if the dataset isn't present, rather than silently training on 0 samples.
    sample_videos = [f for f in os.listdir(args.data_root)
                     if f.endswith((".webm", ".mp4"))] if os.path.isdir(args.data_root) else []
    if not sample_videos:
        print(
            f"\nERROR: No video files found in {args.data_root}\n"
            "The Something-Something V2 dataset must be downloaded first.\n"
            "Run:  python scripts/download_dataset.py\n"
            "See SETUP.MD §8 for details.\n",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    # Mixed precision is CUDA-only. MPS bf16 is unstable for ViT attention.
    ...
    print(f"Building TimeSformer from {args.pretrained}...")
    config = TimesformerConfig.from_pretrained(args.pretrained)
    config.num_labels = NUM_CLASSES
    config.num_frames = args.num_frames
    model = TimesformerForVideoClassification.from_pretrained(
        args.pretrained,
        config=config,
        ignore_mismatched_sizes=True,  # K400 had a 400-class head; ours is 174-class
    ).to(device)
    ...
    run_config = {
        ...
        "gpu_name":         gpu_name,
        "fine_tuning":      "full fine-tune, K400 head replaced with fresh 174-class linear",
        "optimizer":        "AdamW",
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        try:
            ...
            if args.smoke_test:
                print(f"  Val skipped (all batches were missing videos): {e}")
        ...
        with open(os.path.join(RESULTS_DIR, "TimeSformer_results_partial.json"), "w") as f:
    ...
    try:
        ...
        if args.smoke_test:
            print("  Final eval skipped (no valid val samples in smoke-test window).")
            return
        raise
    ...
    # Augment the saved JSON with run config so RESULTS.md can be filled from one file.
    json_path = os.path.join(RESULTS_DIR, "TimeSformer_results.json")
